...
{
    char *lastsep = strrchr(pathname, PATH_SEPARATOR);
...
int main( int argc, char *argv[] )
{
    ...
    ntg_trace_category_bits trace_category_bits = 0;
    ntg_trace_options_bits trace_option_bits = 0;
    ...
    for( i = 1; i < argc; i++ )
    {
        ...
        else
        {
            ...
            {
                trace_option_bits |= TRACE_TIMESTAMP_BITS;
                continue;
            }

            if( strcmp( argument, "-locationstamp_trace" ) == 0 )
            {
                trace_option_bits |= TRACE_LOCATION_BITS;
                continue;
            }

            if( strcmp( argument, "-threadstamp_trace" ) == 0 )
            {
                trace_option_bits |= TRACE_THREADSTAMP_BITS;
                continue;
            }
