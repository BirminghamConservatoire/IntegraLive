...
NTG_HASHTABLE *ntg_hashtable_new()
{
    ...
    hashtable = ntg_malloc( NTG_HASHTABLE_SIZE * sizeof(ntg_hash_node *) );
...
void ntg_hashtable_free(NTG_HASHTABLE *hashtable)
{
	...
	for( i = 0; i < NTG_HASHTABLE_SIZE; i++ )
	{
		while( hashtable[ i ] )
		{
			...
			ntg_free( hashtable[ i ]->key );
			ntg_free( hashtable[ i ] );
...
const void *ntg_hashtable_lookup( NTG_HASHTABLE *hashtable, const void *key, int key_length )
{
    unsigned int hash;
...
void ntg_hashtable_add_key( NTG_HASHTABLE *hashtable, const void *key, int key_length, const void *value )
{
    unsigned int hash;
	ntg_hash_node *node;
	...
	node = ntg_malloc( sizeof( ntg_hash_node ) );
	node->key = ntg_malloc( key_length );
...
void ntg_hashtable_remove_key( NTG_HASHTABLE *hashtable, const void *key, int key_length )
{
    unsigned int hash;
	ntg_hash_node **owner;
	ntg_hash_node *iterator;
...
void ntg_hashtable_add_string_key( NTG_HASHTABLE *hashtable, const char *key, const void *value )
{
	...
	ntg_hashtable_add_key( hashtable, key, strlen( key ), value );
}
...
void ntg_hashtable_add_guid_key( NTG_HASHTABLE *hashtable, const GUID *key, const void *value )
{
	...
	ntg_hashtable_add_key( hashtable, key, sizeof( GUID ), value );
}


void ntg_hashtable_remove_string_key( NTG_HASHTABLE *hashtable, const char *key )
{
	ntg_hashtable_remove_key( hashtable, key, strlen( key ) );
}


void ntg_hashtable_remove_guid_key( NTG_HASHTABLE *hashtable, const GUID *key )
{
	ntg_hashtable_remove_key( hashtable, key, sizeof( GUID ) );
}
...
{
	return ntg_hashtable_lookup( hashtable, key, strlen( key ) );
}


const void *ntg_hashtable_lookup_guid( NTG_HASHTABLE *hashtable, const GUID *key )
{
	return ntg_hashtable_lookup( hashtable, key, sizeof( GUID ) );
}
