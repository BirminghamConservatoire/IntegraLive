...
ntg_xml_sac *ntg_xml_get_sac(const char *schema_path)
{
    ...
    sac = ntg_malloc(sizeof(ntg_xml_sac));
...
ntg_error_code ntg_xml_validate( const char *xml_buffer, unsigned int buffer_length )
{
    ...
    schema_path = ntg_file_find(NTG_SCHEMA_FILE, NTG_XDG_DATA_DIRS + NTG_CWD);

    if (!schema_path) 
	{
        NTG_TRACE_ERROR_WITH_STRING("couldn't find schema", NTG_SCHEMA_FILE);
        return NTG_FAILED;
    }
