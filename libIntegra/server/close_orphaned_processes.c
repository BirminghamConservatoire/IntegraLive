...
char *build_absolute_path(const char *filename)
{
	...
	char *absolute_path = malloc(LONG_STRLEN);
...
void close_orphaned_processes(const char **filenames, int number_of_filenames)
{
	...
	absolute_paths = malloc(number_of_filenames * sizeof(char *));
	absolute_paths_wide = malloc(number_of_filenames * sizeof(WCHAR *));
	...
	for(i = 0; i < number_of_filenames; i++)
	{
		...
		if(absolute_path)
		{
			...
			absolute_paths_wide[number_of_absolute_paths] = malloc(LONG_STRLEN*sizeof(WCHAR));
