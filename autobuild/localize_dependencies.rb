...
# Update input_file paths to local dir (relative to @loader_path)
...
def needs_localization(file_path)
  return file_path.index("@loader_path") == nil && file_path.index("/usr/lib/") == nil
end
...
def update_path(input_file, old_install_name, output_directory)
  command = "install_name_tool -change #{old_install_name} @loader_path/#{File.join(output_directory, File.basename(old_install_name))} #{input_file}"
  puts command
...
def update_id(input_file, output_directory)
  command = "install_name_tool -id @loader_path/#{File.join(output_directory, File.basename(input_file))} #{input_file}"
  puts command
  execute_and_handle(command, nil, nil)
end
