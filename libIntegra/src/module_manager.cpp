...
namespace ntg_internal
{
	...
	void CModuleManager::get_orphaned_embedded_modules( const ntg_node &root_node, guid_set &results )
	{
...
ntg_error_code ntg_module_manager_extract_implementation( ntg_module_manager *module_manager, unzFile unzip_file, const ntg_interface *interface, unsigned int *checksum )
{
	...
	*checksum = 0;
	...
	delete[] implementation_directory;
...
void ntg_module_manager_load_legacy_module_id_file( ntg_module_manager *module_manager )
{
	...
	while( !feof( file ) )
	{
		...
		if( old_id >= module_manager->legacy_module_id_table_elems )
		{
			...
			memcpy( new_legacy_module_id_table, module_manager->legacy_module_id_table, module_manager->legacy_module_id_table_elems * sizeof( GUID ) );
			memset( new_legacy_module_id_table + module_manager->legacy_module_id_table_elems, 0, ( new_table_size - module_manager->legacy_module_id_table_elems ) * sizeof( GUID ) );

			delete module_manager->legacy_module_id_table;
			module_manager->legacy_module_id_table = new_legacy_module_id_table;
			module_manager->legacy_module_id_table_elems = new_table_size;
		}
...
void ntg_module_manager_free( ntg_module_manager *module_manager )
{
	...
	delete[] module_manager->implementation_directory_root;

	ntg_delete_directory( module_manager->embedded_module_directory );
	delete[] module_manager->embedded_module_directory;

	delete[] module_manager->third_party_module_directory;
...
char *ntg_module_manager_get_storage_path( ntg_module_manager *module_manager, const ntg_interface *interface )
{
	...
	module_path = new char[ strlen( storage_directory ) + strlen( unique_name ) + strlen( NTG_MODULE_SUFFIX ) + 2 ];
