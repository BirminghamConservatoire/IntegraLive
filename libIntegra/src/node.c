...
ntg_node *ntg_node_new(void)
{
    ...
    node = ntg_malloc(sizeof(ntg_node));
...
static ntg_path *ntg_node_get_path_helper(const ntg_node * node, ntg_path * path)
{

    path = ntg_path_append_element(path, node->name);
    ...
    return ntg_node_get_path_helper(node->parent, path);
}
...
ntg_path *ntg_node_get_path(const ntg_node * node)
{
    ...
    path = ntg_path_new();
    path = ntg_node_get_path_helper(node, path);
    path = ntg_path_reverse_elements(path);
...
ntg_error_code ntg_node_save_tree( const ntg_node * node, xmlTextWriterPtr writer)
{
	...
	xmlTextWriterWriteFormatAttribute(writer, BAD_CAST NTG_STR_MODULEID, BAD_CAST tmp);
	ntg_free( tmp );
	ntg_free( valuestr );
    ...
    tmp = ConvertInput(valuestr, XML_ENCODING);
	xmlTextWriterWriteFormatAttribute(writer, BAD_CAST NTG_STR_ORIGINID, BAD_CAST tmp);
	ntg_free( tmp );
	ntg_free( valuestr );
...
ntg_error_code ntg_node_save( const ntg_node *node, unsigned char **buffer, unsigned int *buffer_length )
{
	...
	*buffer = ntg_malloc( write_buffer->use );
...
ntg_error_code ntg_node_load( const ntg_node * node, xmlTextReaderPtr reader, ntg_node_list **loaded_nodes )
{
    ...
    while (rv == 1) 
	{
		...
		{
			saved_version = ( char * ) xmlTextReaderGetAttribute( reader, NTG_STR_INTEGRA_VERSION );
			if( saved_version )
			{
        ...
        if (!strncmp((char *)element, NTG_STR_ATTRIBUTE,
            ...
            if (type == XML_READER_TYPE_ELEMENT) {
				...
				existing_attribute = ntg_find_attribute( node, name );
				if( existing_attribute && ntg_endpoint_should_load_from_ixd( existing_attribute->endpoint, attribute_value->type ) )
				{
					...
					*/
					...
					path = ntg_path_append_element(path, (char *)name);
...
const ntg_node_attribute *ntg_find_attribute( const ntg_node *node, const char *attribute_name )
{
	...
	path = ntg_malloc( strlen( node->path->string ) + strlen( attribute_name ) + 2 );
