...
void ntg_scratch_directory_free( ntg_server *server )
{
	...
	ntg_free( server->scratch_directory_root );
...
void ntg_construct_subdirectories( const char *root_directory, const char *relative_file_path )
{
	...
	root_and_subdirectory = ntg_malloc( strlen( root_directory ) + strlen( subdirectory ) + strlen( NTG_PATH_SEPARATOR ) + 1 );
	...
	ntg_free( root_and_subdirectory );
	ntg_free( subdirectory );
}